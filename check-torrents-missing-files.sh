...
do
	percent=`transmission-remote -n "$TRANSMISSION_PASSWORD" -t $TORRENT_ID -i | grep "Percent Done:" | awk '{ print substr($0,index($0,$3)) }'`
	if [ "$percent" != "100%" ]
	then
		continue
	fi
	status=`transmission-remote -n "$TRANSMISSION_PASSWORD" -t $TORRENT_ID -i | grep State: | awk '{ print substr($0,index($0,$2)) }'`
	if [ "$status" != "Stopped" ]
	then
		continue
	fi
	directory=`transmission-remote -n "$TRANSMISSION_PASSWORD" -t $TORRENT_ID -i | grep Location: | awk '{ print substr($0, index($0,$2)) }'`
	name=`transmission-remote -n "$TRANSMISSION_PASSWORD" -t $TORRENT_ID -i | grep Name: | awk '{ print substr($0,index($0,$2)) }'`
	if [ ! -d "$directory" ]
	then
		echo ID: $TORRENT_ID
	...
	size=`transmission-remote -n "$TRANSMISSION_PASSWORD" -t $TORRENT_ID -i | grep "Total size:"`
